...
## README、SKILL、CHANGELOG 和版本说明分别是干什么的
...
| `CHANGELOG.md` | 关心版本账本的人 | 仓库层面实际改了什么 | 不该负责解释一版为什么重要 |
| `docs/v*_版本说明.md` | 想快速理解某一版意义的人 | 这一版主要在补什么缺口、为什么值得关心 | 不该重复逐条充当变更账本 |
| `references/*.md` | 需要深入某一块的人 | 某个专题的稳定参考，例如输出约定、读取顺序、共享约定 | 不该承担第一次上手入口 |
| `docs/完整说明书.md` 等背景文档 | 想看完整背景的人 | 完整说明、语言规范、专题记录、发布包装材料 | 不该替代 README 成为默认入口 |

换句话说：

- `README` 应该让人先读懂“这是什么、有什么用、值不值得继续看”
- `SKILL` 应该让代理知道“该怎么做、先做什么、不能越过什么边界”
- `CHANGELOG` 应该像账本一样记录仓库层面的变化
- `版本说明` 应该解释某一版为什么重要、影响了什么理解或用法
...
这里保留的是简版演进脉络，方便第一次阅读的人快速看懂项目是怎么长出来的。

边界这样分：

- `README` 只保留高层脉络和理解入口
- [CHANGELOG.md](CHANGELOG.md) 记录仓库层面的实际变更账本
- `docs/v*_版本说明.md` 解释某一版为什么重要、改变了什么理解或使用方式

`v0.2.7` 之后尚未正式发布的整理与文档修正，统一记在 [CHANGELOG.md](CHANGELOG.md) 的 `Unreleased`。
