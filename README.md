...
`files-driven` 面向 AI-Native / AI-Driven 项目，帮助团队在项目持续演进中稳住真源、明确下一步、控制改动边界，并让 AI 协作过程可检查、可恢复。它只在规则、文档、状态、流程和工具入口正在阻断交付时才接管治理；默认先恢复交付、融合已有真源、归档历史、冻结过时读取 / 调度信号，再按需修复或审计。
这里的奥卡姆瘦身不是追求更短文本或优先删除；它压缩的是活跃真源数量、重复读取链和越权调度信号。仍在保护写口、停止线或责任归属的边界可以保留，错层内容优先融合、归档或冻结，删除只放最后。

这里的 `Files` 和 `Driven` 要分开理解：
`Files` 承载项目真源、写权、状态、恢复和审计；
`Driven` 承载 agent 执行推进力。
当前 `Superpowers / GSD / gstack / Archon` 这组外部 sidecar 可以先看成 `Driven` 层的四个支柱：执行纪律、长任务推进、虚拟团队质询和 workflow 编排。
...
> 这一类项目在持久化过程中，普遍会遇到漂移、偏离、过度设计、写权失控和恢复困难。本项目的目标是用最少充分控制恢复真实交付，而不是把流程、文书和控制图本身做完整。
...
对下游项目，`files-driven` 作为 meta-skill 的首屏判断先回答：
...
只有在确实要安装或维护 `files engine` 脚手架时，才进入 `install / register / repair / audit`。
...
- 当前场景到底需要多强的控制能力
- 当前规则或约束到底该有多硬、该由什么载体承接
- 应该优先复用宿主原生能力，还是启用脚本、CLI、审批和审计补强

因此，本项目的长期路线是 **用最小充分控制保护交付**，不是把“控制面外移”“Workflow 脚本化”或“CLI backend 化”直接写成长期本体。
后者只是当前版本为解决“复杂流程靠 LLM 内部对话流转时控制不稳”而采用的例外补强策略族。
...
- `L0 -> L4` 只回答执行控制需要多强
- 它不回答一条规则该落在入口、`Skill`、项目边界还是真正的合同面
- 规则问题要另外先做“约束强度与载体”判断
...
1. 先判当前动作是什么
2. 再判当前站在哪个 `scope`
3. 如果当前是规则问题，先判共享层，再判约束强度与载体
4. 再判这轮先停在哪个一级关口
5. 只有需要时，再补四层：`truth_source / execution_object / status_projection / display_projection`
6. 还不够时，才继续下钻结构家族、共享关系和恢复链
...
现在还要再补一句：
先判一条规则该有多硬、该由什么载体承接，再谈它具体进哪个文件。
