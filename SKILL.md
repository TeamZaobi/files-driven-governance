---
name: files-driven
description: 用于 AI-Native / AI-Driven 项目的治理与收口：当项目出现漂移、真源不清、改动边界失控，或需要 install / register / repair / audit 等最小动作时使用；必要时再下钻 capability_scope / project_scope / runtime_scope、入口规则 / 能力规则 / 项目规则 / 项目实体，并在 self-hosting / AI-Native E2E 场景下走 capability-improve + Codex CLI 的受控执行路径或 conversation/replay E2E 验证。
---
...
其中 `AI-Native E2E` 也是并入统一真源的新增验证能力，用来处理正确性开始依赖模型解释、路由、读写边界和恢复判断的场景。
...
- 问题已经进入 `self-hosting`，或者需要 `conversation/replay E2E` 来验证模型判断是否可回放、可恢复、可复核
...
### H. 真源对照、AI-Native E2E 与 self-hosting replay
...
当治理对象已经是 `self-hosting`，或问题明确在 `AI-Native E2E` 上时，默认不再只看 pack / starter 入口，而是转入同一真源下的回放验证口径：先判 `scope`，再看 `route / read / write / boundary / projection / recovery` 是否可回放、可复核、可恢复。

这里的下游会有独立的 `replay/harness starter`，但它承接的是同一套能力模型，不是另一套世界观。
