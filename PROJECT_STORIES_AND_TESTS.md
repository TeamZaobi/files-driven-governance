...
- 仓库维护者或 AI coding 代理需要处理运行中反复出现的纠偏信号，但不能直接热改能力真源，因此要先把观察保留为候选，再决定是否晋升为能力。
- 下游项目的采用者或 AI coding 代理，希望拿到一条官方 starter 闭环，能把 `files-driven` 作为 `meta-skill` 安装进新项目，而不是继续从 example 和说明文里反推起步路径。
- 通过 Agent 使用这个 skill 的人，希望自然语言请求能稳定命中正确 reference/example 路由，并按正式输出骨架收到结果，而不是先自己逆向 repo 结构。

## 当前交付物

- 一套可以公开理解、可以本地验证、也可以持续演进的 `files-driven` 共享存储治理能力基线：
  - 中文主叙事入口
  - 问题导向的团队使用手册
  - 项目治理能力模型统一真源
  - `v1` 历史阶段兼容入口
  ...
  - 运行观察 -> 候选保留 -> 能力晋升 的官方 reference、example pack 和专项回归
  - 当前阶段补完计划
  - Agent-facing 路由与正式输出骨架回归
  - validator 与最小回归测试
...
- 这次完成后，用户应该看到什么变化：新接手的人可以直接解释这个仓库现在是“以 [docs/项目治理能力模型.md](docs/项目治理能力模型.md) 为唯一底层真源、以 `v1 -> v2 -> v2.1` 为能力模型演进轴、并附带 governed pack / contract `tranche v1` 最小合同链”的共享存储项目治理能力项目，而不是泛化成任何文档治理都要全包的大全集。
...
- 为什么这件事对当前阶段重要：当前项目仍在从“共享存储治理说明”推进到“最小可执行能力”，最容易出现的风险就是新增很多资产，却没有共同服务同一条交付线。
...
- 谁在用：负责维护运行中纠偏、候选保留和能力晋升边界的人。
- 在什么场景下：运行日志、临时修补和重复纠偏已经开始积累，但当前回合不能热改能力真源。
- 他/她现在想完成什么：先把这些信号停在运行记录，必要时压成证据包、召回历史、拆分出口，再决定是否进入候选试验或晋升到能力真源。
- 为什么这件事对当前阶段重要：如果每次纠偏都直接写回真源，能力模型会被运行噪声拖着走，项目边界会越来越不稳。
- 这次完成后，用户应该看到什么变化：团队可以明确说明哪些信号只做记录、哪些进入候选保留、哪些满足跨场景验证后才晋升。
- 这次明确不包含什么：不要求把所有运行观察自动升级成能力，也不要求让每次临时纠偏都立即回写真源。

### 用户故事 US-14

- 谁在用：通过 Agent 使用这个 skill 的人，以及维护 agent 入口资产的人。
- 在什么场景下：用户直接给 Agent 一句自然语言请求，期望 skill 自己完成路由、引用正确 reference/example，并输出稳定骨架。
- 他/她现在想完成什么：不用先理解 repo 结构，也能让 `files-driven` 把请求稳定落到正确主路径，并按正式输出约定回答。
- 为什么这件事对当前阶段重要：如果 Skill 的实际使用界面是 Agent，而仓库只测试 CLI 和 pack，最后就会出现“资产都对、真实使用还是飘”的断层。
- 这次完成后，用户应该看到什么变化：常见用户原话已经和 `agents/openai.yaml`、`SKILL.md`、`references/输出约定.md`、官方 reference/example 形成一条可回归的 agent-facing 链。
- 这次明确不包含什么：不要求这一阶段就引入真实在线模型、多 agent 协同或不稳定外部工具调用的黑盒回放。
...
- 对应故事：US-13
- 前提：某个运行场景里反复出现同类纠偏，但维护者不能直接热改能力真源。
- 当：从 `README.md` 的新入口进入，再读 `docs/使用手册.md`、`references/运行观察与能力晋升.md`、`examples/capture-candidate-activation/README.md`，必要时再回看 `references/经典治理流程库.md`。
- 那么：应能判断这条信号先停在运行记录、候选保留还是能力晋升，并说清为什么不能把临时纠偏直接写回能力真源。
- 通过条件：维护者能用仓库内主路径解释“先保留观察，再压成证据包和历史召回，拆成项目出口与能力候选，最后决定试验、激活或回退”，而不是把每次纠偏都当成立即改真源的理由。
- 这次明确不要求：不要求在首次阅读时就完成完整候选评审细则或跨场景验证清单。
- 失败/越界边界：如果看完入口后仍然只能回答“问题出现了”，却说不清该停在哪一层、何时进入候选、何时才可晋升，说明这条能力还没有被项目边界接住。

### 测试用例 TC-15

- 对应故事：US-14
- 前提：用户是通过 Agent 而不是直接读 repo 文件来使用这个 skill。
- 当：给出自然语言请求，例如“我要搭一个 AI Agent 驱动的新项目”“这条议题还不适合进 task”“运行里同类纠偏反复出现，但我不想直接热改能力真源”“我的目标读者几乎没有软件工程基础”。
- 那么：`agents/openai.yaml`、`SKILL.md`、官方 reference/example 和 [references/输出约定.md](references/输出约定.md) 应共同形成一条稳定链路，把请求路由到正确资产，并给出正确输出骨架。
- 通过条件：维护者无需调用真实在线模型，也能验证 Agent 入口 prompt、技能路由和正式输出骨架没有脱节。
- 这次明确不要求：不要求这条回归替代真实 runtime 黑盒测试；这里只守 agent-facing skill contract。
- 失败/越界边界：如果用户原话已经在 README 或 SKILL 里出现，但 Agent 入口 prompt 没把它接进正式输出骨架，或路由资产仍然互相脱节，说明这个 skill 还没有把 Agent 界面守住。
