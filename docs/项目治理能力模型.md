...
因此，本项目的长期路线是 **用最小充分控制保护交付**，
而不是把“控制面外移”“Workflow 脚本化”或“LLM 调度 Backend 化”直接写成长期本体。
这些都只是当前阶段为了修复控制失稳而采用的实现策略；如果轻控制、宿主原生能力和现有真源已经足够，就不应升级到更重的控制面。

奥卡姆剃刀是当前能力模型的默认制动规则：

这里的“瘦身”不是按字数删薄，也不是把边界说明赶出真源。
它要压缩的是活跃真源数量、重复读取链和越权调度信号。
边界可以讲，只要它服务于守住写口、停止线或责任归属；不能为了显得更薄而删除仍在保护交付的边界。
处理一段材料时，先判断它还属于当前真源、应该融合回其他已有真源、只应归档为历史，还是需要冻结其活跃读取 / 调度权；只有明显错误、未生效或无保留价值时，才删除。

1. 能直接交付、测试或给出唯一阻断对象时，不新建治理对象。
2. 能融合到现有真源、归档为历史证据、冻结活跃读取 / 调度权、复用现有真源或改现有一行时，不新建文件、状态、gate、workflow、runner 或控制面；删除只用于明显错误、未生效或无保留价值的对象。
3. 新增控制对象前，必须说明它替代、合并、降级或退役哪个旧信号；不能退役旧信号时，默认不新增。
4. validator pass、owner verdict、Dashboard 更新、review-ready 和 candidate package 只算证据信号；只有 tested change、user-visible delivery、accepted implementation surface 或 single blocker path 才算推进。
5. docs-only 闭包只能跑一轮；之后必须转为 `open_implementation`、`blocked_one_evidence` 或 `no_more_docs`，不得继续生成同层 packet、dossier、owner-verdict 或 index。
...
1. 先识别用户要交付什么、当前被什么阻断
2. 能直接交付、测试、融合、归档、冻结或复用现有真源时，先走轻动作；删除放在最后
3. 宿主原生 `workflow / subagents / approvals` 足够时优先复用
4. 只有当复杂流程需要编排、恢复、回放、审计，且轻治理已经失败时，才启用脚本 runner、`CLI` 节点执行或其他外置控制补强
