...
放到项目治理里，它至少要帮助用户回答四件事：

1. 当前真正的问题是什么
2. 失控点出在真源、流程、角色边界还是 runtime
3. 当前场景需要多强的控制能力
4. 应该优先依赖宿主原生能力，还是启用脚本、`CLI`、审批或审计补强
...
### 8.1 `AI-Native E2E` 已并进同一能力模型

`AI-Native E2E` 不是一份平行真源，也不只属于 `Skill` 驱动项目。
它是当前能力模型里的正式验证能力，用来处理所有“正确性已经开始依赖大模型能力”的工作方式，例如：

1. `Skill` 驱动
2. `Prompt / Rules / Agent` 驱动
3. 受控 workflow / agent 协同
4. 多工具、多代理共享仓库的 AI-Native 协作

它的启用条件不是“仓库里有没有 `Skill`”，
而是：

**只要系统正确性已经不再只取决于确定性代码执行，而开始取决于模型的解释、路由、读取选择、写入边界、投影约束或恢复判断，就必须引入 `AI-Native E2E`。**

因此，它和普通实现测试、schema/validator 测试、合同一致性测试的关系是分工，不是替代：

1. 确定性实现测试继续负责固定输入、固定输出、固定校验逻辑
2. schema / validator 测试继续负责结构正确性和合同边界
3. `AI-Native E2E` 负责验证模型参与判断后，治理链有没有失真
4. 多 runtime 一致性测试负责验证不同入口和执行器不会把同一请求解释成不同治理语义

这里的 `adapter contract` 绑定的是“项目当前选定的合适 CLI / runtime”，
不是仓库级固定某一个工具。
`kimicc`、`codex`、`claude` 或其他项目自选执行器，都只能作为 adapter 实例出现。

`AI-Native E2E` 的一等断言对象当前冻结为：
...
这里的 `AI-Native E2E` 仍然挂在 `workflow / harness` 这条验证面上，
但它不要求仓库去发明第二套字段体系。
当前最小合同仍复用已有 tranche v1 的脚手架锚点；
例如 replay / harness starter 会继续沿用 `workflow_contract_path` 这类字段名来承接 `harness.contract.json`，
同时继续复用当前的 `primary_gate` 枚举，而不是另起一套平行 gate 语义。

### 8.2 `AI-Native E2E` 的建设顺序

这项能力必须按串行主线建设，不能先让实现细节反过来定义方法：

1. 先并进统一能力模型，冻结适用范围、启用条件、一等断言对象和失真分类
2. 再冻结 `case / replay artifact / assertion report / run metadata / adapter` 合同族
3. 再建立至少一条可执行的 `self-hosting conversation/replay E2E`
4. 再把这套能力压成下游可 adopt 的 `schema + starter + runner/validator 接口 + fixture 规则`
5. 最后才扩到多 runtime、一致性、并发和高压力矩阵

只有在第二步完成后，下面这些工作才适合并行展开：

1. schema 补齐
2. replay / harness starter 骨架
3. runner / adapter / assertion report 接口
4. fixture 与失败样本整理
5. 下游 adopt 示例

终局的高压力矩阵至少应覆盖：

1. 短指令歧义与多 workflow 混触发
2. 长对话与上下文压缩
3. 跨 scope 污染
4. 中断恢复与续跑
5. 多工作站 / 多代理并发
...
10. `schemas/ai-native-e2e.*.schema.json`：`AI-Native E2E` 的 case / replay / report / run metadata / adapter 合同层
11. `starters/ai-native-e2e-replay-harness/`：面向下游项目的 replay / harness starter
12. `tests/test_ai_native_e2e_contracts.py` 与后续 replay validator：负责 `AI-Native E2E` 合同链和 self-hosting 回放的机器校验
...
8. `AI-Native E2E` 是否只在“正确性开始依赖模型能力”时启用，而没有被误写成脚本测试替代品
9. replay / harness starter 是否仍然复用统一 tranche 兼容面，而没有偷长出第二套 schema / gate 体系
10. self-hosting 是否至少存在一条可执行的 `conversation/replay E2E`
11. 高压力矩阵是否仍然落回 `route / read / write / boundary / projection / recovery / trajectory integrity`
