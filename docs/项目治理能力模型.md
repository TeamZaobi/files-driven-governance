...
7. `AI-Native E2E` 是否只在“正确性开始依赖模型能力”时启用，而没有被误写成脚本测试替代品
8. replay / harness starter 是否仍然复用统一 tranche 兼容面，而没有偷长出第二套 schema / gate 体系
9. self-hosting 是否至少存在一条可执行的 `conversation/replay E2E`
10. 高压力矩阵是否仍然落回 `route / read / write / boundary / projection / recovery / trajectory integrity`
