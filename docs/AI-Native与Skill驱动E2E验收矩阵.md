...
如果下游项目反馈“测试目标不够直接、不够准”，默认先补这套协议。

一句话通过线：

**E2E 通过 = 真实入口触发 + 真源读取正确 + 写权受控执行 + 冷启动可恢复重放 + 结果 oracle 一致。**

这里显式区分：

1. `AI kernel`：负责“读对、判对”（上下文链）
2. `control plane`：负责“写对、可恢复”（写权链与恢复链）

任何一条 lane，如果只测了前者或只测了后者，都不算完整 E2E。

### 4.1 每条 E2E 用例必须写明的 8 个字段

| 字段 | 作用 | 最低要求 |
| --- | --- | --- |
| `entrypoint` | 真实入口，不用口头流程替代 | 必须是可执行命令或可重放 runner 入口 |
| `truth_source_allowlist` | 限定允许读取的真源 | 至少列出本次判定必读真源文件 |
| `projection_denylist` | 禁止把投影当真源 | 至少列出状态/展示类禁止升格文件 |
| `write_allowlist` | 限定可写路径 | 至少覆盖本次运行会写入的路径 |
| `write_role` | 限定可写角色 | 必须与 contract/规则声明一致 |
| `oracle` | 明确“通过到底长什么样” | 至少包含 `gate_state` 与 `next_step` 或等价状态锚点 |
| `replay_seed` | 冷启动恢复最小种子 | 至少覆盖可恢复当前判断所需的最小文件集 |
| `failure_boundary` | 失败/越界触发条件 | 至少一条“出现什么就判失败” |

### 4.2 推荐样式（可直接给下游）

```toml
[e2e_case]
case_id = "runtime-pack-001"
lane = "runtime promotion chain"
entrypoint = "python scripts/manage_files_engine.py audit <project> --layer runtime --format json"
truth_source_allowlist = ["BOUNDARY.md", "workflow.contract.json", "objects/*.json"]
projection_denylist = ["status.projection.json", "display/*.md"]
write_allowlist = ["workflow.state.json", "workflow.events.jsonl"]
write_role = "maintainer"
oracle = { gate_state = "ready", next_step = "transition.capture-to-candidate" }
replay_seed = ["BOUNDARY.md", "workflow.contract.json", "workflow.state.json", "workflow.events.jsonl"]
failure_boundary = "任何 projection 文件被当作控制真源，或重放后 gate_state 不一致"
```
...
1. 测试描述只有“应该更好/应该可用”，没有 oracle
2. 只写“先读文档”，没有真实入口命令
3. 没写写权边界，导致测试隐含默认可写任何文件
4. 没有 replay seed，导致当前回合过了但下一轮不可恢复
5. 结果只校验 prose，不校验状态锚点或 runner 产物
## 5. 当前结论
