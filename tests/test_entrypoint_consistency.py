...
class EntrypointConsistencyTests(unittest.TestCase):
    def test_governed_pack_entrypoints_keep_canonical_terms(self) -> None:
        ...
        self.assertIn("conversation/replay E2E", quickstart)
        self.assertIn("replay/harness starter", quickstart)
        ...
        self.assertIn("ai-native-e2e.assertion-report.schema.json", schema)
    ...
    def test_metadata_matches_skill_default_path_language(self) -> None:
        ...
        self.assertIn("AI-Native E2E", metadata)
        self.assertIn("conversation/replay E2E", metadata)
        ...
        self.assertIn("对话层展示链接", metadata)
        self.assertIn("桌面端可点击文件链接", metadata)
        self.assertIn("不要把桌面端可点击绝对路径文件链接直接落盘成项目 Markdown 内链", metadata)
