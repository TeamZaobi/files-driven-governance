...
class EntrypointConsistencyTests(unittest.TestCase):
    def test_governed_pack_entrypoints_keep_canonical_terms(self) -> None:
        ...
        self.assertIn("用最小充分控制保护交付", readme)
        self.assertIn("不是追求更短文本或优先删除", readme)
        self.assertIn("压缩的是活跃真源数量、重复读取链和越权调度信号", readme)
        ...
        self.assertIn("用最小充分控制保护交付", skill)
        self.assertIn("不是按字数删薄", skill)
        self.assertIn("边界可以讲，只要它服务于守住写口、停止线或责任归属", skill)
        self.assertIn("把错层材料回指或融合到正确已有真源", skill)
        ...
        self.assertIn("用最小充分控制保护交付", model)
        self.assertIn("不是按字数删薄", model)
        self.assertIn("不能为了显得更薄而删除仍在保护交付的边界", model)
    ...
    def test_metadata_matches_skill_default_path_language(self) -> None:
        ...
        self.assertIn("最小治理 skill", metadata)
        self.assertIn("用最小充分控制保护交付", metadata)
        self.assertIn("哪份文件算数 / 今天先做哪一步 / 哪些先别改", metadata)
        self.assertIn("install / register / repair / audit", metadata)
        self.assertIn("回答骨架是：结论 -> 最小动作 -> 验证证据 -> 停止线", metadata)
        self.assertIn("Obsidian / Notion / Docs / Sheets / Slides", metadata)
        self.assertIn("治理问题还是工具操作问题", metadata)
        self.assertIn("真源、写权、投影、证据冲突、恢复、读取顺序或重复文书", metadata)
        self.assertIn("audit 默认只是分层基础体检", metadata)
        self.assertIn("最多读一个专题 reference", metadata)
        self.assertIn("FilesStuck", metadata)
        self.assertIn("不是按字数删薄", metadata)
        self.assertIn("压缩的是活跃真源数量、重复读取链和越权调度信号", metadata)
        self.assertIn("不是默认入口", metadata)
